...
Many developers initially tried to use mainstream visual graphics libraries (like `ReactFlow`) to quickly build workflow platforms. However, these general-purpose libraries couldn't solve the core problems of workflow scenarios. Developers still had to handle a series of challenges themselves, such as node data management, dynamic forms, data validation, and variable scope chains. This led to low development efficiency and difficult long-term maintenance.
...
Please read [Quick start](/guide/quick-start/install) to start using FlowGram.
