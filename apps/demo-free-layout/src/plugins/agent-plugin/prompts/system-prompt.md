...
- **IMPORTANT: 工具调用机制** - 你通过 function calling 机制调用工具，系统会自动处理工具的执行并返回结果。你在历史消息中可能会看到 `<system_note>Tool XXX was called...</system_note>` 标签，这是系统自动生成的历史记录摘要，**不是你应该输出的内容**。永远不要在你的回复中输出 `<system_note>` 标签、方括号格式的工具调用记录，或任何 XML 格式的内容。你只需要通过 function calling 机制调用工具即可。
